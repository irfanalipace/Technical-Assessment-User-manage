...
export default function TaskManager({ contactId }: { contactId: string }) {
  ...
  const saveEdit = async (id: string) => {
    const updated = {
      // ✅ Specify the type of t here
      tasks: tasks.map((t: Task) => (t.id === id ? { ...t, title: editTitle } : t)),
    };
    mutate(updated, false);
    try {
      await fetch(`/api/tasks/${id}`, {
  ...
  const toggle = async (id: string, current: boolean) => {
    const updated = {
      tasks: tasks.map((t: Task) => (t.id === id ? { ...t, completed: !current } : t)),
    };
    mutate(updated, false);
    try {
      await fetch(`/api/tasks/${id}`, {
  ...
  const remove = async (id: string) => {
    mutate({ tasks: tasks.filter((t: Task) => t.id !== id) }, false);
    try {
      await fetch(`/api/tasks/${id}`, { method: 'DELETE' });
      mutate();
    } catch (e) {
